...
output_size = 1  # 输出层维度
lr = 0.001  # 学习率
train_range = 1000  # 重复训练次数

# ——————————————————导入数据——————————————————————
f = open('./data/single/redeem_1408.csv')
df = pd.read_csv(f)
data_8 = np.array(df['redeem'])
print(data_8)

f = open('./data/single/redeem_1403to1408.csv')
df = pd.read_csv(f)
data_38 = np.array(df['redeem'])
normalize_total = (data_38 - np.mean(data_38)) / np.std(data_38)  # 标准化
normalize_total = normalize_total[:, np.newaxis]  # 增加维度

f = open('./data/single/' + train_type + '.csv')
df = pd.read_csv(f)
data_37 = np.array(df['redeem'])
        ...
        for i in range(train_range):
            ...
            while (end < len(train_x)):
                ...
                if step % 10 == 0:
                    ...
                    print("model_save", saver.save(sess, test_path+"/redeem_save1/model.ckpt"))
                step += 1
...
def prediction():
    ...
    with tf.Session() as sess:
        # 参数恢复
        module_file = tf.train.latest_checkpoint(test_path+"/redeem_save1")
        ...
        plt.plot(list(range(len(normalize_data), len(normalize_data) + len(normalize_predict))), normalize_predict,
                 color='r')
        plt.savefig(test_path+"/test_date=" + str(test_date) + "_single_" + train_type + "_ts=" + str(time_step) + "_bs=" + str(
            batch_size) + "_lr=" + str(
        ...
        # 计算error
        redeem_error = np.divide(np.abs(data_8 - predict.flatten()), data_8)
        redeem_score = 0
        ...
        print('redeem_score:\n', redeem_score)
        text_file = open("./test/redeem_score.txt", "a+")
        text_file.write(
        ...
        predict.to_csv(
            test_path+"/test_date=" + str(test_date) + "_single_" + train_type + "_ts=" + str(time_step) + "_bs=" + str(batch_size) + "_lr=" + str(
