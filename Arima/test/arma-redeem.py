...
redeem_arma_model = sm.tsa.ARMA(redeem_data1403to1407, (redeem_p, redeem_q)).fit()
...
plt.legend()
plt.title('8\'s redeem predict by arma')
plt.draw()
plt.savefig(
...
text_file.write('redeem(arma) p:%d q:%d time:%f score:%f' % (
    redeem_p, redeem_q, end - start, redeem_score))
text_file.write('\n')
text_file.close()
# 显示图像
plt.show()
