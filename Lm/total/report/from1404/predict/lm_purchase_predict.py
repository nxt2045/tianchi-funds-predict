...
df = pd.read_csv(r"../../data/processed/lm_1404to1409.csv", sep=',', engine='python',
                 encoding='utf-8',
...
print(y_pred)
result = pd.DataFrame(y_pred, columns=['purchase'], index=data_test[x_cols].index)
result.to_csv(save_path + 'purchase.csv')

y_pred_all = model.predict(df[x_cols].values)

# 画预测图
y_pred_all = pd.DataFrame(y_pred_all, columns=['purchase'], index=df.index)

y_pred_all.to_csv(save_path + 'purchase_all.csv')
